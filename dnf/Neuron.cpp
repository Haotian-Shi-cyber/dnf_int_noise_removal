...
Neuron::Neuron(int _nInputs)
{
	...
	initialWeights = new int[nInputs];
	inputs = new long[nInputs];
}
...
void Neuron::initNeuron(int _neuronIndex, int _layerIndex, weightInitMethod _wim, biasInitMethod _bim, actMethod _am){
	...
	for (int i=0; i<nInputs; i++) {
		...
		case W_ONES:
			...
			break;
		case W_ONES_NORM:
			weights[i] = int(1.0 / (double)nInputs * 65536.0);
			break;
		case W_RANDOM:
			weights[i] = int(((( (double)rand() / RAND_MAX)) - 0.5) * 65536.0);
			break;
		case W_RANDOM_NORM:
			weights[i] = int(((( (double)rand() / RAND_MAX)) - 0.5) / (double)nInputs * 65536.0);
			break;
		}
	...
	switch (_bim){
		...
		break;
        case B_RANDOM:
		bias= int(((( (double)rand() / (RAND_MAX))) - 0.5) * 65536.0);
		break;
	}
}

void Neuron::setLearningRate(int _w_learningRate, int _b_learningRate){
...
void Neuron::setError(int _backwardError){
...
int Neuron::getWeightChange(){
	weightsDifference = 0;
	weightChange = 0;
	for (int i=0; i<nInputs; i++){
		weightsDifference = weights[i] - initialWeights[i];
		weightChange += int(pow(weightsDifference,2));
	}
	return (weightChange);
}

int Neuron::getWeightDistance(){
	return int(sqrt(weightChange));
}
...
int Neuron::getWeights(int _inputIndex){
	assert((_inputIndex>=0)&&(_inputIndex<nInputs));
	return (weights[_inputIndex]);
}

int Neuron::getInitWeights(int _inputIndex){
	assert((_inputIndex>=0)&&(_inputIndex<nInputs));
