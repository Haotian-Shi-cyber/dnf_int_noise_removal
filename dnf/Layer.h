...
public:
	 ...
	 **/
	void setlearningRate(int _w_learningRate, int _b_learningRate);
	
	/**
	 * Sets the inputs to all neurons in the first hidden layer only
	 * @param _inputs A pointer to an array of inputs
	 */
	void setInputs(const int *_inputs, const int scale = 1, const unsigned int offset = 0, const int n = -1);
	 ...
	 */
	int getOutput(int _neuronIndex);
	 ...
	 */
	long getSumOutput(int _neuronIndex);
	 ...
	 */
	int getWeights(int _neuronIndex, int _weightIndex);
	 ...
	 */
	int getGlobalError(int _neuronIndex);
	 ...
	 * @return
	 */
	int getInitWeight(int _neuronIndex, int _weightIndex);
